...
export async function POST(req: Request) {
  ...
  try {
    const [statsRes, daysRes, expRes, eventsRes, settingsRes] = await Promise.all([
      sql`
      ...
      `,
      sql`SELECT * FROM monthly_expenses LIMIT 6`,
      sql`SELECT date, type, description, impact FROM events ORDER BY date DESC LIMIT 10`,
